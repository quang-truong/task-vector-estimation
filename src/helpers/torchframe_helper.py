#############################################################################
##### A Joblib Version of TorchFrame for Faster Data Processing ###########
#############################################################################
...
class Dataset(ABC):
    ...
    def materialize(
        ...
        # 1. Fill column statistics in parallel:
        def compute_col_stats_for_col(col, ser, stype, sep, time_format):
            return col, compute_col_stats(
                ser,
                stype,
                sep=sep,
        ...
        non_text_cols = [(col, stype) for col, stype in self.col_to_stype.items() if stype != torch_frame.text_embedded]
        text_cols = [(col, stype) for col, stype in self.col_to_stype.items() if stype == torch_frame.text_embedded]
        
        # Joblib parallel for non-text columns
        parallel = ProgressParallel(n_jobs=num_workers, use_tqdm=True, total=len(non_text_cols))
        non_text_results = parallel(
            delayed(compute_col_stats_for_col)(
                col,
                self.df[col],
                stype,
                self.col_to_sep.get(col, None),
                self.col_to_time_format.get(col, None),
            )
            for col, stype in non_text_cols
        )
        
        text_results = []
        for col, stype in text_cols:
            text_results.append(compute_col_stats_for_col(
                col,
                self.df[col],
                stype,
                self.col_to_sep.get(col, None),
        ...
        # Combine results
        results = non_text_results + text_results
        ...
        for col, stats in results:
            ...
            # tasks.
            if col == self.target_col and self.col_to_stype[col] == torch_frame.categorical:
