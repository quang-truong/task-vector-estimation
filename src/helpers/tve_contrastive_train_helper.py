...
def train(cfg, model, task, task_name, data_loader, optimizer, loss_fn, col_stats_dict, device, **kwargs):
    model.train()
    loss, sce_loss, contrastive_loss = train_one_epoch(cfg, model, task, data_loader, optimizer, loss_fn, col_stats_dict, device, **kwargs)
    if get_world_size() > 1:
        ...
        reduced_contrastive_loss = torch.tensor(contrastive_loss).to(device)
        dist.all_reduce(reduced_contrastive_loss, op=dist.ReduceOp.SUM)
        reduced_contrastive_loss = reduced_contrastive_loss / get_world_size()
        reduced_contrastive_loss = reduced_contrastive_loss.item()
    else:
        reduced_loss = loss
        reduced_sce_loss = sce_loss
        reduced_contrastive_loss = contrastive_loss
        
    loss_dict = {
        "total_loss": reduced_loss,
        "sce_loss": reduced_sce_loss,
        "contrastive_loss": reduced_contrastive_loss,
    }
    return {task_name: loss_dict}

def train_one_epoch(cfg, model, task, data_loader, optimizer, loss_fn, col_stats_dict, device, **kwargs):
    loss_accum = count_accum = sce_loss_accum = contrastive_loss_accum = 0
    steps = 0
    ...
    for batch in tqdm(data_loader, total=total_steps, desc=f"Training rank {rank}..."):
        ...
        pred_out, aug_out, out = model(batch, task.entity_table)
        
        loss, sce_loss, contrastive_loss = loss_fn(pred_out, batch[task.entity_table].y.float(), aug_out, out)
        ...
        contrastive_loss_accum += contrastive_loss.detach().item() * batch_size
        count_accum += batch_size
    
        steps += 1
        if steps > cfg.train.max_steps_per_epoch:
            break
    return loss_accum / count_accum, sce_loss_accum / count_accum, contrastive_loss_accum / count_accum

@torch.no_grad()
def test(cfg, model, task, task_name, data_loader, loss_fn, col_stats_dict, device):
    model.eval()
    loss, sce_loss, contrastive_loss = test_one_epoch(cfg, model, task, data_loader, loss_fn, col_stats_dict, device)
