        ...
        **kwargs,
        ...
        for node_type in data.node_types:
            for col_name in col_stats_dict[node_type].keys():
                ...
                # elif torch_frame.timestamp in data[node_type].tf.col_names_dict and col_name in data[node_type].tf.col_names_dict[torch_frame.timestamp]:
                #     num_time_feats = 7 * conv_kwargs["channels"]
                #     self.recon_head[node_type][col_name] = MLP(conv_kwargs["channels"], out_channels=num_time_feats, norm="layer_norm", num_layers=1)
        ...
        if self.training:
            for node_type, tf in batch_clone.tf_dict.items():
                for stype in tf.stypes:
                    ...
                    #     # feat shape: (batch_size, num_cols, 7)
                    #     feat = batch_clone[node_type].tf.feat_dict[stype]
                    #     shuffle_cols = []
                    #     for i in range(feat.size(1)):
                    #         feat_col = feat[:, i, :]                        # shape: (batch_size, 7)
                    #         shuffle_cols.append(feat_col[torch.randperm(feat_col.size(0))])
                    #     shuffle_feat = torch.stack(shuffle_cols, dim=1)     # shape: (batch_size, num_cols, 7)
                    #     mask_matrix = (torch.rand(feat.size(0), feat.size(1) * feat.size(2), device=feat.device) < self.mask_rate).int()
                    #     new_feat = mask_matrix * shuffle_feat.view(feat.size(0), -1) + (1 - mask_matrix) * feat.view(feat.size(0), -1)
                    #     new_feat = new_feat.view(feat.size(0), feat.size(1), feat.size(2))
                    #     batch_clone[node_type].tf.feat_dict[stype] = new_feat
