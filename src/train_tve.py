...
from src.helpers.model_helper import get_conv_layer, get_hetero_encoder
from src.helpers.task_helper import get_task_metadata
from src.helpers.save_helper import save_backbone_checkpoint, save_checkpoint, load_checkpoint
from src.helpers.tve_train_helper import train, test

from src.models.tve import TVE
...
from src.utils.log_utils import CurvesLogger, print_epoch_metrics
...
def main(cfg, cfg_file, sweep_file, sweep_folder_name=None):
    ...
    for run in range(cfg.num_runs):
        ...
        print(f"Loaded TVE({', '.join([f'{k}={v}' for k, v in cfg.model.items()])})")
        ...
        for epoch in range(0, cfg.train.num_epochs):
            ...
            if epoch % cfg.train.eval_every == 0:
                # load_checkpoint(model, optimizer, os.path.join(checkpoint_dir, f'model-epoch{epoch+1}.pth'), device)
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.sweep_config):        # run sweep files
        raise NotImplementedError("Sweep files are not supported for pre-training.")
